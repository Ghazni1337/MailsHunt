...
class PaypalController extends Controller
{
	...
	public function expressCheckout(Request $request) {
	  ...
	  if (!$response['paypal_link']) {
	    return response()->json(['success' => false, 'message' => 'Something went wrong with PayPal']);
	...
	private function getCart($recurring, $invoice_id)
    {
        ...
        if ($recurring) {
            return [
                ...
                'items' => [
                    [
                        'name' => $request->title.'Monthly Subscription ' . config('paypal.invoice_prefix') . ' #' . $invoice_id,
                        'price' => $request->price,
                        'qty' => 1,
                    ],
                ],
                ...
                'subscription_desc' => $request->title.'Monthly Subscription ' . config('paypal.invoice_prefix') . ' #' . $invoice_id,
                ...
                'total' => $request->price*1, // Total price of the cart
            ];
        }
    ...
    public function expressCheckoutSuccess(Request $request) {
        ...
        if (!in_array(strtoupper($response['ACK']), ['SUCCESS', 'SUCCESSWITHWARNING'])) {
            return response()->json(['success' => false, 'message' => 'Error processing PayPal payment']);
        }
        ...
        if ($invoice->paid) {
            return response()->json(['success' => true, 'message' => 'Order ' . $invoice->id . ' has been paid successfully!']);
        }
        
        return response()->json(['success' => false, 'message' => 'Error processing PayPal payment for Order ' . $invoice->id . '!']);
    }
}
